//
// Created by emiel on 10/07/20.
//

#ifndef SPECTRALHOOK_NETVARS_H
#define SPECTRALHOOK_NETVARS_H

#include <unordered_map>
#include <cstdint>
#include <cstring>
#include "../include/tf2/Classes/RecvTable.h"
#include "../include/tf2/Classes/ClientClass.h"
...
public:
    static uintptr_t GetOffset(const char* class_name, const char* property_name, RecvProp** property_ptr = nullptr) {
        ...
        for (ClientClass* class_ptr = g_clientdll->GetAllClasses(); class_ptr; class_ptr = class_ptr->m_pNext) {
            if (strcmp(class_ptr->m_chName, class_name) == 0) {
                uintptr_t result = FindOffset(class_ptr->m_recvTable, property_name, property_ptr);
